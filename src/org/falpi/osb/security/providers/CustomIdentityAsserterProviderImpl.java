...
public final class CustomIdentityAsserterProviderImpl implements AuthenticationProviderV2, IdentityAsserterV2 {
   ...
   public CallbackHandler assertIdentity(String StrTokenType, Object ObjToken, ContextHandler ObjContext) throws IdentityAssertionException {
      ...
      // Verifica se il token in ingresso Ë un BASIC o un JWT
      ...
      if (StrAuthType.equals(TokenTypes.JWT_AUTH_ID)) {   
         ...
         // Verifica se la chiave Ë gi‡ in cache         
         HashMap ObjKey = (HashMap) ObjJwtKeysCache.get(StrJwtKeyID);
         
         // Se la chiave Ë in cache e il suo timestamp non Ë scaduto esegue altrimenti procede
         if ((ObjKey!=null)&&
            ...
            try {
               ...
               // Se non Ë stata trovata alcuna chiave genera eccezione
         ...
         // Se l'asserzione di identit‡ non Ë definita esegue, altrimenti procede
         ...
         // Se non Ë richiesto il mapping esegue, altrimenti procede
         if (StrJwtIdentityMappingMode.equals("DISABLE")) {
            ...
            try {               
               ...
               // Se il mapping Ë fallito genera eccezione
      ...
      if (!StrValidationAssertion.equals("")) {
         ...
         try {
            // Se l'espressione di validazione Ë positiva genera logging altrimenti eccezione
   ...
   public Object evaluateScript(String StrScript, String StrClassName) throws Exception {
      ...
      // Se la tipologia di oggetto restituita non Ë quella attesa genera eccezione
   ...
   public String getHttpResource(String StrRequestURL, 
      ...
      // Se non Ë stato allocato alcun login context kerberos esegue, altrimenti procede
      if (ArrLoginContext.size() == 0) {
         ...
         // Se c'Ë stata eccezione la genera
      ...
      // Se lo statuscode Ë diverso da 200 genera eccezione
   ...
   public void LogMessage(int IntLevel,String StrMessage,Object ObjDetails) {
      ...
      if (Math.max(IntLoggingLevel,IntLoggingLevelMin)<=IntLevel) {
         ...
         // Se il suffisso non Ë stato popolato e c'Ë una eccezione prova a estrapolare la prima dello stacktrace
